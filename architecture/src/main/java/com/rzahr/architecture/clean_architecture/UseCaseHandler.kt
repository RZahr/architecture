...
class UseCaseHandler() {
    ...
    suspend fun <T : UseCase.RequestValues, R : UseCase.ResponseValue> execute(useCase: UseCase<T, R>, values: T, callback: UseCase.UseCaseCallback<R>) {
        ...
        withContext(Dispatchers.Main) { useCase.useCaseCallback?.onLoading() }
        useCase.run()
    }
    suspend fun <T : UseCase.RequestValues, R : UseCase.ResponseValue> executeWithLiveData(UseCaseN: UseCase<T, R>, values: T, liveData: LiveData<LiveDataState<R>>) {

        UseCaseN.requestValues = values
        UseCaseN.useCaseCallbackN = UiCallbackWrapperN(liveData)
        withContext(Dispatchers.Main) { UseCaseN.useCaseCallbackN?.onLoading() }
        UseCaseN.run()
    }
